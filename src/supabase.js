...
// ── Thumbnails IG (reutiliza la tabla "campaigns" con flag "ig_thumbnails") ──
// Sube todas las imágenes a Storage; la DB solo guarda URLs.
export async function saveIgSession(session) {
  ...
  let screenshot = session.screenshot || null;
  let thumbnails = Array.isArray(session.thumbnails) ? session.thumbnails : [];

  try {
    const [shot, thumbs] = await Promise.all([
      screenshot ? uploadImageToStorage(screenshot, { folder: "ig", userId: uid }).catch(() => screenshot) : null,
      Promise.all(thumbnails.map((t) =>
        t?.image
          ? uploadImageToStorage(t.image, { folder: "ig", userId: uid })
              .then((url) => ({ ...t, image: url }))
              .catch(() => t)
          : Promise.resolve(t)
      )),
    ]);
    ...
    console.error("saveIgSession: error subiendo imágenes —", e.message);
  }

  const thumb = thumbnails.find?.((t) => t.image)?.image || screenshot || null;
  ...
  const { data: row, error } = await supabase
    ...
    .insert({
      user_id:      uid,
      product_name: session.analysis?.accountName || session.analysis?.handle || "Thumbnails IG",
      niche:        "ig_thumbnails",
      city:         session.analysis?.niche || null,
      thumbnail:    thumb,
      data:         payload,
    })
    .select()
  ...
  if (error) { console.error("saveIgSession:", error.message); return null; }
  return row;
}

export async function listIgSessions() {
  if (!supabase) return [];
  const { data, error } = await supabase
    .from("campaigns")
    .select("id, created_at, product_name, niche, city, thumbnail, data")
    .eq("niche", "ig_thumbnails")
    .order("created_at", { ascending: false })
    .limit(50);
  if (error) { console.error("listIgSessions:", error.message); return []; }
  return data || [];
}

export async function getIgSession(id) {
  if (!supabase) return null;
  const { data, error } = await supabase
    .from("campaigns").select("*").eq("id", id).single();
  if (error) { console.error("getIgSession:", error.message); return null; }
  return data;
}
